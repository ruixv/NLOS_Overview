...
def main() -> None:
    ...
    for key, doi in zip(KEYS, DOIS):
        if merged.count(key) != 1:
            raise RuntimeError(f"merged bibliography key inconsistency for {key}")
        doi_field_count = len(re.findall(rf"(?im)^\s*doi\s*=\s*\{{{re.escape(doi)}\}}\s*,?\s*$", merged))
        if doi_field_count != 1:
            raise RuntimeError(
                f"merged bibliography DOI-field inconsistency for {key} / {doi}: {doi_field_count}"
            )
