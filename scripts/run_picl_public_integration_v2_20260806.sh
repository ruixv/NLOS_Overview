...
# Latest Additions row that caused its legacy one-occurrence assertion to fail.
set +e
...
note = note_path.read_text(encoding="utf-8")nnote = note.replace(
...
# Execute the already reviewed build, validation, rendering, and generated-branch
