...
if not pdf.exists() or pdf.stat().st_size < 100_000:
    raise SystemExit("bare_jrnl.pdf was not generated or is unexpectedly small")
...
missing = [phrase for phrase in required if phrase not in compact]
if missing:
    raise SystemExit("Expected PDF text missing: " + ", ".join(missing))
print(f"Validated {len(reader.pages)} PDF pages and all integrated records.")
