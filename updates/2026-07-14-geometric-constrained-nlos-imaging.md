...
2. **index.html** — enrich the existing searchable paper object and expand the 2025 timeline with geometric surface-normal priors; retain the existing count of 96 tracked entries.
...
The synchronization script is marker-based and idempotent. It aborts when an expected insertion anchor is missing or duplicated rather than replacing large files blindly. The validation workflow records completion only after the clean PDF build and consistency checks pass.
