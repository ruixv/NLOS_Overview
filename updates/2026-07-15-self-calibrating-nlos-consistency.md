...
The high-priority forward-citation pass identified a cross-artifact consistency gap for:
...
The title was already present in `README.md` and the homepage paper explorer, so the update did not increment the tracked-entry count. Before this run:
...
The marker-based script `scripts/sync_nlos_20260715_selfcal.py`:

- updated the README and homepage metadata to **ACM TOG / SIGGRAPH Asia 2023** and the DOI link;
- preserved the homepage tracked-entry count at **96**;
- added an explicit 2023 milestone for end-to-end self-calibrating differentiable NLOS inverse rendering;
- inserted the paper into the active-SPAD reconstruction table;
- added a semantic paragraph immediately after the fast differentiable transient-rendering discussion;
- merged `egbib_20260715_selfcal_updates.bib` into the duplicate-free bibliography;
- clean-built and validated `bare_jrnl.pdf`.

The workflow rejected missing anchors, non-idempotent edits, undefined citations, missing BibTeX records, repeated BibTeX entries, empty PDFs, failed text extraction, and a rebuilt PDF with fewer rendered pages than the previous version. All checks passed.
