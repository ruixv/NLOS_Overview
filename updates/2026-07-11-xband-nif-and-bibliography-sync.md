...
**Build result:** successful clean LaTeX/BibTeX rebuild from the case-normalized, duplicate-free consolidated bibliography. `bare_jrnl.pdf` was regenerated; `pdfinfo`/`pdftotext` passed, the final LaTeX log contains no undefined citations/references, BibTeX reports no missing or repeated keys, and both new paper titles were verified in the PDF text (latexmk exit code 0).
