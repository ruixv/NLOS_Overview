...
**Build result:** successful clean LaTeX/BibTeX rebuild. README, homepage, survey source, duplicate-free bibliography, and `bare_jrnl.pdf` are synchronized. `pdfinfo`/`pdftotext` passed, with no undefined or repeated citations (latexmk exit code 0).
