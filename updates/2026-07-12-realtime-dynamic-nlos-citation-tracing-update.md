...
- `egbib_20260712_realtime_updates.bib`: publisher-verified metadata under `namRealTimeDynamicScenes2021`.
...
**Build result:** successful clean LaTeX/BibTeX rebuild. README, homepage, survey source, duplicate-free bibliography, and `bare_jrnl.pdf` are synchronized. `pdfinfo`/`pdftotext` passed and no undefined or repeated citation records were detected (latexmk exit code 0).
